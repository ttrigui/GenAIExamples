...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor):
        orig_type = x.dtype
        ret = nn.LayerNorm.forward(self, x.to(torch.float32))
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False):
        if return_hidden:
            hidden = self.visual(image.to(self.dtype), return_hidden)
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        pos_emd = self.positional_embedding[:x.size(1), :].to(self.dtype)
        x = x + pos_emd
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
    ...
    def forward(self, image, text, flops=False):
        if flops:
...
def build_model(state_dict: dict, load_state_dict: bool=False, fp16=True):
